...
**Operating body:** 이음통합평생교육원 (separate site: www.ieumedu.kr)
...
- **Related site:** www.ieumedu.kr — 이음통합평생교육원 (parent org, separate operation)
...
**Logo / wordmark:** The live site does not carry a graphical logo. The brand mark is typographic: **"이음교육저널"** set in Noto Serif KR 600, letter-spacing `-0.025em`, with the tagline *"교육 · 심리 온라인 저널"* in sans 13px beneath. There is a rounded `46×46px` square mark in the current redesign (gradient from `--color-accent` to `#97AE9F`) — this is documented in the UI kit as the **placeholder mark** and should be treated as non-final.
...
- `assets/` — real article imagery from the live site (for use in mocks)
...
- **Imagery:** eight images lifted from the live codebase are included. They are documentary photos with no obvious license note in the repo; treat as "brand-owned" pending confirmation.
- **Logo:** no final logotype exists. The UI kit uses the same typographic wordmark + gradient mark placeholder the live site uses.
- **No full "production" design system:** this project documents foundations + one UI kit (the news site). The parent-org site (www.ieumedu.kr) is explicitly out of scope per the source brief.
